...
def test_open_event_roundtrip(conn):
    event_id = db.open_event(
        conn, metric="co2", tier="ceiling", opened_at=NOW,
        value=1400.0, baseline=520.0, threshold=1200.0, notified=True,
    )
...
def test_close_event_removes_from_open(conn):
    event_id = db.open_event(
        conn, metric="co2", tier="relative", opened_at=NOW,
        value=900.0, baseline=500.0, threshold=800.0, notified=True,
    )
...
def test_update_peak_keeps_maximum(conn):
    event_id = db.open_event(
        conn, metric="voc", tier="relative", opened_at=NOW,
        value=800.0, baseline=200.0, threshold=500.0, notified=True,
    )
...
def test_mark_renotified(conn):
    event_id = db.open_event(
        conn, metric="pm25", tier="ceiling", opened_at=NOW,
        value=50.0, baseline=5.0, threshold=35.0, notified=True,
    )
